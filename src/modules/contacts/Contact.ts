...
export default class Contact {
    ...
    async createContact(contactInfo: ContactCreateAttributes) {
        const result = await this.apiClient.post<ApiResponse<ContactAttributes>, ContactCreateAttributes>(
    ...
    async updateContact() {
        const result = await this.apiClient.post<ApiResponse<ContactAttributes>>("/connect/v1.0/contacts")
        return result.data;
    }
}